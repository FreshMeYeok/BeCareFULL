...
class CustomDataset(Dataset):
    ...
    def __init__(self, metadata_file, frames_folder, transform=None):
        ...
        print("metadata 갯수")
        splitlen = 15
        print(len_metadata//splitlen - len_metadata//40)
        # 읽어온 JSON file로 데이터 전처리
        for k, obj in enumerate(metadata):
            if not(k > len_metadata//40 and k <= len_metadata//splitlen):
                continue
            obj_name = obj
            ...
            for frame_idx in range(anomaly_start - anomaly_start//3, num_frames - read_image_count):
                frame_path_list = []
                # two_frame = []
